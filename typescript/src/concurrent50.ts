...
async function worker(i: number, label: string, url: string, total?: number) {
  const outDir = path.join(path.dirname(new URL(import.meta.url).pathname), `../results/concurrent_50_${label}`);
  ensureDir(outDir);
...
async function chooseSiteInteractive(): Promise<[string, string]> {
  ...
  try {
    console.log('Choose a site to screenshot concurrently (50 shots):');
